...
import {ABI_JSON} from './busdc.abi'
...
export const functions = {
    ...
    burn: new Func<[amount: bigint], {amount: bigint}, boolean>(
        abi, '0x42966c68'
    ),
    ...
    initialize: new Func<[name: string, symbol: string, decimals: number, amount: bigint, mintable: boolean, owner: string], {name: string, symbol: string, decimals: number, amount: bigint, mintable: boolean, owner: string}, []>(
        abi, '0xef3ebcb8'
    ),
    mint: new Func<[amount: bigint], {amount: bigint}, boolean>(
        abi, '0xa0712d68'
    ),
    mintable: new Func<[], {}, boolean>(
        abi, '0x4bf365df'
    ),
...
export class Contract extends ContractBase {
    ...
    mintable(): Promise<boolean> {
        return this.eth_call(functions.mintable, [])
    }
